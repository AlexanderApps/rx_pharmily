  ...
  const handleUseCurrentLocation = async () => {
    ...
    try {
      ...
      try {
        ...
        toast.success(
          "Location captured. Add an address label if you'd like."
        );
      }
    } catch (err) {
      toast.error(
        "Couldn't get your current location. Check location permissions and try again."
      );
    } finally {
      setCapturing(false);
    }
  ...
  return (
    <View>
      ...
      <Pressable
        ...
        disabled={capturing}
        className="flex-row items-center gap-1.5 self-start mt-2 py-1.5 px-2.5 border rounded-lg active:opacity-70"
        style={{
      ...
      {hasGpsFix && (
        <View
          className="flex-row items-center gap-1.5 self-start mt-2 py-[5px] px-[9px] rounded-lg"
        ...
        >
          ...
          >
            GPS location attached ({latitude!.toFixed(4)},{" "}
            {longitude!.toFixed(4)})
          </Text>
          {onLocationCleared && (
