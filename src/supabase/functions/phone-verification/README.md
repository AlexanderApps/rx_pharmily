...
(e.g. `+233241234567`) and rejects landline numbers. Both are now
enforced by `checkPhoneNumber()` (`phone-number.ts`, via
`libphonenumber-js`) — `handleStart` runs it on the entity's stored
phone number before calling the provider at all, so a malformed or
landline number never reaches (and never costs) a real Prelude call.
The same check runs client-side too
(`shared/utils/phone-number.ts`, in `ProfileUpdateRequestModal`) so
someone gets this feedback immediately when entering a number, rather
than only after an admin later approves it. The two files are kept in
sync by hand, not shared as one module — see either file's own comment
for why (Deno vs React Native are genuinely separate runtimes here).

## Code length

Prelude's `code_size` option defaults to whatever an account's
Dashboard is configured to (documented range: 4-8 digits) if left
unset — this produced a real, live bug: an account configured for 4
digits sent a 4-digit code while every other part of this app (the UI,
the dummy provider) assumed exactly 6. `PreludeVerificationProvider`
now explicitly requests `code_size: 6` on every `start()` call rather
than depending on that account-level default, and the client UI
(`phone-verification-sheet.tsx`) accepts any length in Prelude's own
4-8 range regardless, so this can't silently break again the same way
