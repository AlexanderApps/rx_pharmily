...
export class PreludeVerificationProvider implements VerificationProvider {
  ...
  async start(phone: string): Promise<StartVerificationResult> {
    // Phone number is normalized to E.164 by checkPhoneNumber()
    // (phone-number.ts) before this is ever called — see handlers.ts.
    //
    // options.code_size is explicit here, not left to the account's
    // Dashboard default — Prelude's own docs put that default anywhere
    // from 4 to 8 digits depending on account configuration, and
    // leaving it unset produced exactly that inconsistency in practice
    // (a 4-digit code arriving when the rest of this app — the dummy
    // provider included — assumes 6). Forcing 6 here keeps the UX
    // predictable regardless of what the Dashboard happens to be set
    // to, for this account or any other Prelude account this code
    // might run against later.
