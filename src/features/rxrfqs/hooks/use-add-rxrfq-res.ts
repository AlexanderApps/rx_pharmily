...
const INITIAL_FORM_STATE: RxRfqResponseFormData = {
  id: "",
  paymentTerms: "",
  vendorComment: "",
  status: "draft",
...
export default function useAddRxRfqResponse(
  ...
  const baselineData = useRef<RxRfqResponseFormData>({
    ...INITIAL_FORM_STATE,
    ...initialData,
  });

  const [formData, setFormData] = useState<RxRfqResponseFormData>(
    baselineData.current,
  );
  const [errors, setErrors] = useState<
    Partial<Record<keyof RxRfqResponseFormData, string>>
  >({});
  ...
  const updateField = useCallback(
    <K extends keyof RxRfqResponseFormData>(field: K, value: RxRfqResponseFormData[K]) => {
      setFormData((prev) => ({
        ...prev,
        [field]: value,
      }));
  ...
  const addItem = useCallback((item: RxRfqResponseItem) => {
